# scrapes HTML minutes, 1993-2019, from FOMC and tokenizes + removes stopwords, then scores by FSS algorithm (londono et al.)
...
dfFSS = pd.DataFrame({'date': timeline, 'score': scores})
plt.figure()
plt.bar(np.linspace(1990,2019,106), dfFSS.score, width = 0.1)
dfFSS.to_csv('FSS_scores_all.csv', index = False)
